...
char *fs_cd (char *dir)
{
	FRESULT res;
		 ...
		 if (dir == NULL) {
			   ...
			   res = f_chdir(dir);  // acceder a un repertoire
	...
	if (!card_ready) return -1;
	 ...
	 FRESULT res;
	   ...
	   if (!card_ready) return -1;
		 ...
		 res = f_mkdir(dirname);  // creer une repertoire
...
int fs_rm(char* filename)
{
	FRESULT res;
		 ...
		 res = f_unlink(filename); // suprimer la fichier de la repertoire
		if(res != FR_OK){
...
int _write(int fd, char *ptr, int len) {
	...
	/* A COMPLETER */
	UINT bw;
		res = f_write(&files[v], ptr, len, &bw);
			    if(res != FR_OK){
			        	return -1;
			        }
			    return bw;
...
******/
{
	...
	/* setup sd card */
	sd_init(&card,card_detect_cb, NULL);  // faire l'appel a Sd_init
     //////////////
